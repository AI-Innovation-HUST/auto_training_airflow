...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, n_blocks=6, d_model=512, d_feature=64,
        ...
        self.high_layer = nn.Sequential(nn.Linear(92*16,256),
                                        nn.Dropout(0.1),
                                        nn.Linear(256,1))
        self.low_layer = nn.Sequential(nn.Linear(92*16,256),
                                       nn.Dropout(0.1),
                                       nn.Linear(256,1))
    ...
    def forward(self, x: torch.FloatTensor, 
        ...
        # print(x.shape)
        x = torch.flatten(x,1)
        high = self.high_layer(x)
        low = self.low_layer(x)
...
class WordPositionEmbedding(nn.Module):
    ...
    def forward(self, x: torch.LongTensor, mask=None) -> torch.FloatTensor:
        ...
        b = self.position_embedding(x.long())
